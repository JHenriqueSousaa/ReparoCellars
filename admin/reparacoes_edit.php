...
if($_SERVER["REQUEST_METHOD"]=="POST") {
    ...
    */
    ...
    if(empty($idReparacao)) {
        ...
        // query para editar um alunos existente
        $query = "update Reparacoes set CodigoReparacao='$codigoReparacoes', idCliente='$clienteReparacoes', Equipamento='$equipamentoReparacoes', IMEI='$imeiReparacoes', Obs='$obsReparacoes', EstadoReparacao='$estadoReparacoes', DescricaoEstado='$descricaoReparacoes' where idReparacao= '$idReparacao'";

        // executamos a consulta
        $resultado = mysqli_query($conexao, $query);

        // se retornar um true encaminhamos para a página clientes msg=2
        if($resultado) {
...
<div class="container">
        ...
        <form method="POST" action="">
            <div class="form-group row">
                ...
                <div class="col-sm-7">
                <input type="text" name="CodigoReparacao" class="form-control" id="CodigoReparacao" placeholder="Códiogo de Reparação" value="<?=$codigoReparacoes?>" readonly>
                </div>
            </div>
            <div class="form-group row">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-7">
                <input type="text" name="IMEI" class="form-control" id="IMEI" placeholder="IMEI" value="<?=$imeiReparacoes?>" pattern="[0-9]{15}" title="Apenas usar digitos. Formato: 000000000000000 (15)">
                </div>
            </div>
            <div class="form-group row">
            ...
            <div class="form-group row">
                ...
                <select class="form-control" name="EstadoReparacao" id="EstadoReparacao">
                    <option value=1 <?php if($estadoReparacoes==1) echo "selected";?>>Em Aberto</option>
                    <option value=2 <?php if($estadoReparacoes==2) echo "selected";?>>Para aprovação</option>
                    <option value=3 <?php if($estadoReparacoes==3) echo "selected";?>>Em reparação</option>
                    <option value=4 <?php if($estadoReparacoes==4) echo "selected";?>>Pronto para entrega</option>
                    <option value=5 <?php if($estadoReparacoes==5) echo "selected";?>>Entregue</option>
                    <option value=6 <?php if($estadoReparacoes==6) echo "selected";?>>Cancelada</option>
                </select>
                </div>
            </div>
            <div class="form-group row">
